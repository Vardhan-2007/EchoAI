...
@media (max-width:767px) {
  ...
  /* FIX: don't constrain SVG/img with * selector — only block-level content */
...
// ─── MODAL (FIXED) ────────────────────────────────────────────────────────────
// KEY CHANGE: overlay scrolls (overflowY:auto), card anchored at top (alignItems:flex-start).
// This means the title field is always the first thing visible — no more clipping.
  ...
  const overlay = (
    ...
    >
      ...
      >
        ...
        {/* Body — natural height, no scroll trap */}
        <div
          ref={bodyRef}
...
function Dashboard({tasks, habits, focusSess, setView}) {
  ...
  const total=tasks.length, pct=total?Math.round((done/total)*100):0;
...
// ─── FORM FIELD WRAPPER — defined OUTSIDE TaskManager so React doesn't remount
// inputs on every keystroke (defining components inside another component causes
// React to see a new type each render → unmount/remount → focus lost after 1 char)
...
// ─── TASK MANAGER ─────────────────────────────────────────────────────────────
// EMPTY must be outside TaskManager — if defined inside, every render creates
// a new object reference which can cause unnecessary re-renders
const TASK_EMPTY = {title:"",desc:"",priority:"medium",cat:"Work",due:today(),status:"todo",notes:"",quad:"Q2",urgency:5,importance:5,aiRationale:""};
...
function TaskManager({tasks, setTasks}) {
  ...
  const save=useCallback(()=>{
    ...
    if (editId){setTasks(t=>t.map(x=>x.id===editId?{...form,id:editId}:x));setEditId(null);}
    else{setTasks(t=>[...t,{...form,id:uid()}]);}
    setForm(TASK_EMPTY);setModal(false);setError("");
  ...
  const startEdit=useCallback(task=>{setForm({...TASK_EMPTY,...task});setEditId(task.id);setModal(true);},[]);
  ...
  const parseNL=useCallback(async()=>{
    ...
    try {
      ...
      setForm(f=>parsed?.title?{...TASK_EMPTY,...f,...parsed}:{...TASK_EMPTY,title:nlInput});
    } catch { setForm({...TASK_EMPTY,title:nlInput}); }
    setNlInput(""); setParsing(false); setModal(true);
  },[nlInput]);

  const filtered=useMemo(()=>tasks.filter(t=>filter==="all"?true:filter==="done"?t.status==="done":t.status!=="done"),[tasks,filter]);

  return (
    <div className="anim" style={{display:"flex",flexDirection:"column",gap:18}}>
      <SectionHeader title="Task Manager" actions={<button className="btn btn-p" onClick={()=>{setEditId(null);setForm(TASK_EMPTY);setModal(true);}}>{I.plus} New Task</button>}/>
      ...
      <div style={{display:"flex",gap:7,alignItems:"center"}}>
        {["all","active","done"].map(f=>(
          <button key={f} className={`btn btn-sm ${filter===f?"btn-p":"btn-ghost"}`} onClick={()=>setFilter(f)}>{f.charAt(0).toUpperCase()+f.slice(1)}</button>
        ))}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:7}}>
        {filtered.length===0&&<EmptyState icon="📋" title="No tasks" body="Add tasks above or use natural language."/>}
        {filtered.map(t=>{
          const q=t.quad?QUADS[t.quad]:null;
          return (
            <div key={t.id} className="card-sm" style={{padding:"12px 15px",display:"flex",alignItems:"center",gap:11,opacity:t.status==="done"?.55:1}}>
              <button style={{width:17,height:17,borderRadius:5,background:t.status==="done"?"var(--ok)":"transparent",border:`1.5px solid ${t.status==="done"?"var(--ok)":"var(--b2)"}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,color:"#fff",transition:"var(--transition)"}} onClick={()=>toggle(t.id)}>{t.status==="done"&&I.chk}</button>
              <div style={{flex:1,minWidth:0}}>
...
const loadFromStorage=()=>{try{const r=localStorage.getItem(STORAGE_KEY);return r?JSON.parse(r):null;}catch{return null;}};
const saveToStorage=h=>{try{localStorage.setItem(STORAGE_KEY,JSON.stringify(h));}catch{}};
const resetStreaksIfVersionMismatch=habits=>{
  const v=localStorage.getItem(STORAGE_VERSION_KEY);
  if (v===CURRENT_VERSION) return habits;
  localStorage.setItem(STORAGE_VERSION_KEY,CURRENT_VERSION);
  const reset=habits.map(h=>({...h,streak:0,days:Array(7).fill(false)}));
  saveToStorage(reset); return reset;
};
const applyStreakReset=habits=>{
  const lastOpen=localStorage.getItem(LAST_OPEN_KEY);
  const todayD=dateStr(), yesterday=yesterdayStr();
  localStorage.setItem(LAST_OPEN_KEY,todayD);
  if (lastOpen===todayD) return habits;
  if (!lastOpen) return habits;
  const missedYesterday=lastOpen!==yesterday;
  const yesterdayIdx=new Date(yesterday).getDay();
  return habits.map(h=>{
    const days=Array.isArray(h.days)&&h.days.length===7?h.days.map(Boolean):Array(7).fill(false);
    const shouldReset=missedYesterday||!days[yesterdayIdx];
    return {...h,days,streak:shouldReset?0:(typeof h.streak==="number"&&h.streak>=0?h.streak:0)};
  });
};
...
function HabitTracker({habits:propHabits, setHabits:setPropHabits}) {
  ...
  const [habits,setHabitsLocal]=useState(()=>{
    ...
    return applyStreakReset(resetStreaksIfVersionMismatch(base));
  });
  const setHabits=useCallback(updater=>{
    setHabitsLocal(prev=>{const next=typeof updater==="function"?updater(prev):updater;saveToStorage(next);setPropHabits?.(next);return next;});
  },[setPropHabits]);
  const safeHabits=useMemo(()=>habits.map(h=>({...h,streak:typeof h.streak==="number"&&h.streak>0?h.streak:0,days:Array.isArray(h.days)&&h.days.length===7?h.days.map(Boolean):Array(7).fill(false)})),[habits]);
  const add=useCallback(()=>{
    if (!newName.trim()) return;
    setHabits(h=>[...h,{id:uid(),name:newName.trim(),emoji:newEmoji||"⭐",streak:0,days:Array(7).fill(false),createdAt:dateStr()}]);
  ...
  const toggleDay=useCallback((hid,di)=>{
    ...
    setHabits(h=>h.map(x=>{
      ...
      const days=Array.isArray(x.days)&&x.days.length===7?[...x.days]:Array(7).fill(false);
      days[di]=!days[di];
      const cur=typeof x.streak==="number"&&x.streak>=0?x.streak:0;
      return {...x,days,streak:days[di]?cur+1:Math.max(0,cur-1)};
    }));
  },[todayIdx,setHabits]);
  const del=useCallback(id=>setHabits(h=>h.filter(x=>x.id!==id)),[setHabits]);
  const weeklyData=useMemo(()=>DAYS.map((d,i)=>({day:d,done:safeHabits.filter(h=>h.days[i]===true).length})),[safeHabits]);
  return (
    <div className="anim" style={{display:"flex",flexDirection:"column",gap:18}}>
        ...
        <div className="card" style={{padding:20}}>
          ...
          <ResponsiveContainer width="100%" height={96}>
            <BarChart data={weeklyData} barSize={16}>
              ...
              <Bar dataKey="done" radius={[4,4,0,0]}>{weeklyData.map((_,i)=><Cell key={i} fill={i===todayIdx?"var(--ac)":"var(--s5)"}/>)}</Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
        ...
        <div key={h.id} className="card-sm" style={{padding:"15px 18px"}}>
          <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:13}}>
            ...
            <div style={{flex:1}}>
              ...
              {h.streak>0?<div style={{fontSize:11.5,color:"var(--warn)",marginTop:2}}>🔥 {h.streak} day streak</div>:<div style={{fontSize:11.5,color:"var(--t3)",marginTop:2}}>Start your streak today!</div>}
            </div>
            <div style={{display:"flex",alignItems:"center",gap:9}}>
              <div className="pbar" style={{width:64}}><div className="pfill" style={{width:`${Math.min(100,(h.streak/30)*100)}%`,background:"var(--warn)"}}/></div>
              <button className="btn btn-ghost btn-sm" style={{color:"var(--err)"}} onClick={()=>del(h.id)}>{I.trash}</button>
            </div>
          </div>
          <div style={{display:"flex",gap:5}}>
            {DAYS.map((d,i)=>{
              const isToday=i===todayIdx, isDone=h.days[i]===true;
              return (
                <div key={d} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:5}}>
                  <span style={{fontSize:9,color:isToday?"var(--ac2)":"var(--t3)",fontWeight:isToday?700:400}}>{d}</span>
                  <button disabled={!isToday} onClick={()=>toggleDay(h.id,i)}
                    style={{width:25,height:25,borderRadius:7,background:isDone&&isToday?"var(--ok)":"transparent",border:`1.5px solid ${isDone&&isToday?"var(--ok)":isToday?"var(--ac)":"var(--b1)"}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:isToday?"pointer":"not-allowed",opacity:isToday?1:.35,outline:isToday?"2px solid rgba(124,92,252,.2)":"none",outlineOffset:2,transition:"var(--transition)",color:"#fff"}}>
                    {isDone&&I.chk}
                  </button>
                </div>
              );
...
function Analytics({tasks, habits, focusSess}) {
  ...
  const topStreak=useMemo(()=>Math.max(...habits.map(h=>h?.streak||0),0),[habits]);
  const score=useMemo(()=>{if(!tasks.length)return 0;return Math.round((done/tasks.length)*40+Math.min(40,(totalFocusMins/60)*8)+Math.min(20,(habits.filter(h=>h?.days?.[new Date().getDay()]).length/Math.max(habits.length,1))*20));},[done,tasks.length,totalFocusMins,habits]);
      ...
      <div className="card" style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{padding:"11px 15px",borderTop:"1px solid var(--b1)"}}>
          <div style={{display:"flex",gap:5,marginBottom:9,flexWrap:"wrap"}}>
            {SUGGESTIONS.map(s=><button key={s} className="btn btn-ghost btn-sm" style={{fontSize:11,color:"var(--t2)"}} onClick={()=>!loading&&send(s)}>{s}</button>)}
          </div>
          <div style={{display:"flex",gap:8}}>
